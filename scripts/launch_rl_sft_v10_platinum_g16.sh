...
config="$project_root/configs/prime-rl/rl-sft-v10-platinum-g16-v1.toml"
...
mode="${1:-start}"
default_name="bird-rl-sft-v10-plat-full-g16-100step-$(date +%Y%m%d-%H%M%S)"
...
case "$mode" in
  ...
  *)
    printf 'Usage: %s [check|smoke|start] [run-name] [target-steps]\n' "$0" >&2
    exit 2
    ;;
esac
...
if [[ "$mode" == "smoke" ]]; then
  run_name="${2:-bird-rl-sft-v10-plat-full-g16-smoke-$(date +%Y%m%d-%H%M%S)}"
  max_steps=2
  extra_args="--orchestrator.eval.skip-first-step True"
elif [[ "$mode" == "resume" ]]; then
  max_steps="${target_steps:-240}"
  extra_args="--resume"
else
  max_steps="${target_steps:-100}"
  extra_args=""
fi
...
printf 'Started RLVR %s in tmux session: %s\n' "$mode" "$session_name"
printf 'Run: %s\n' "$run_name"
