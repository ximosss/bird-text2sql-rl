...
candidate_model_id="${BIRD_CANDIDATE_MODEL_ID:-bird-revisql-step1300}"
session_name="${BIRD_TMUX_SESSION:-bird-revisql-step1300-final-eval-$(date +%Y%m%d-%H%M%S)}"
models_url="http://127.0.0.1:8000/v1/models"
uv_cache_dir="${UV_CACHE_DIR:-/tmp/bird-text2sql-rl-uv-cache}"
run_prefix="${BIRD_RUN_PREFIX:-bird-revisql-step1300}"
...
for benchmark_name in "${benchmark_names[@]}"; do
  case "$benchmark_name" in
    bird-mini-dev|bird-full-dev|arcwise-plat-sql|arcwise-plat)
      configs+=("$project_root/configs/prime-rl/eval/revisql-$benchmark_name.toml")
      ;;
    *)
...
for required in "$prime_rl_root" "$base_model" \
  "$adapter/adapter_model.safetensors" "$adapter/adapter_config.json" "${configs[@]}"; do
  [[ -e "$required" ]] || {
...
for index in "${!configs[@]}"; do
  ...
  printf -v eval_command \
    '%s && env UV_CACHE_DIR=%q LOCAL_API_KEY=local NO_PROXY=%q no_proxy=%q uv run --no-sync eval @ %q --model %q --run.name %q' \
    "$eval_command" "$uv_cache_dir" "$no_proxy_value" "$no_proxy_value" \
    "${configs[$index]}" "$candidate_model_id" "$run_name"
done
...
printf 'Started ReViSQL final evals in tmux session %s\n' "$session_name"
for benchmark_name in "${benchmark_names[@]}"; do
