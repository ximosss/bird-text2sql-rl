...
session_name="${BIRD_TMUX_SESSION:-bird-base-final-eval-$(date +%Y%m%d-%H%M%S)}"
...
for benchmark_name in "${benchmark_names[@]}"; do
  case "$benchmark_name" in
    bird-mini-dev|bird-full-dev|arcwise-plat-sql|arcwise-plat)
      configs+=("$project_root/configs/prime-rl/eval/$benchmark_name-base.toml")
      ;;
    *)
...
for required in "$prime_rl_root" "$base_model" "${configs[@]}"; do
  if [[ ! -e "$required" ]]; then
...
if ! nvidia-smi -L >/dev/null 2>&1; then
  printf 'No NVIDIA GPU is visible; refusing to start the formal evaluation.\n' >&2
  exit 1
fi
...
printf -v server_command \
  'exec env CUDA_VISIBLE_DEVICES=0 UV_CACHE_DIR=%q NO_PROXY=%q no_proxy=%q uv run --no-sync vllm serve %q --host 127.0.0.1 --port 8000 --served-model-name %q --reasoning-parser qwen3 --max-model-len 36864 --gpu-memory-utilization 0.9 --generation-config vllm' \
  "$uv_cache_dir" "$no_proxy_value" "$no_proxy_value" "$base_model" "$base_model_id"
...
while ((SECONDS < deadline)); do
  ...
  if payload="$(curl --noproxy '*' --silent --fail --max-time 3 "$models_url" 2>/dev/null)" && \
    jq -e --arg model "$base_model_id" '.data | any(.id == $model)' <<<"$payload" >/dev/null; then
    break
  fi
  sleep 2
done
...
for index in "${!configs[@]}"; do
  run_name="bird-base-${benchmark_names[$index]}--$batch_id"
  printf -v eval_command \
    '%s && env UV_CACHE_DIR=%q LOCAL_API_KEY=local NO_PROXY=%q no_proxy=%q uv run --no-sync eval @ %q --run.name %q' \
    "$eval_command" "$uv_cache_dir" "$no_proxy_value" "$no_proxy_value" "${configs[$index]}" "$run_name"
done
...
printf 'Started base final evals in tmux session %s\n' "$session_name"
for benchmark_name in "${benchmark_names[@]}"; do
  printf 'Run: bird-base-%s--%s\n' "$benchmark_name" "$batch_id"
done
printf 'Results: %s\n' "$output_root"
